...
| `hooks/` | C source for native DLLs | `rawinput_hook.c` (input bypass) + `damage_hook.c` (damage multiplier via vendored MinHook), compiled to `rawinput_hook.dll` / `damage_hook.dll` by build.ps1 / CI; `build.ps1` |
| `vendor/minhook/` | Vendored MinHook v1.3.4 sources (MIT) | `include/MinHook.h`, `src/{hook,buffer,trampoline}.c`, `src/hde/hde64.c` — compiled together with `hooks/damage_hook.c`; license in `LICENSE.txt` |
...
| `js/risk-gate.js` | **Shared** risk-service gate module (`RISK_SERVICES` registry + global `RiskGate`): normalized disclaimer text (common bullets + per-service line, single source of truth), consent persistence via `{service}.disclaimer_accepted` config keys, first-entry overlay gating for risk pages (`gatePage`), in-place consent modal for Launcher-config checkboxes (`gateLauncherSection` + `showConsentModal`), a view-only re-read modal (`showNoticeModal`), and `refreshLauncherVisibility()` — services flagged `hideUntilConsent` (currently `damage_hook`) stay hidden on the Launcher-config page until consent is given on the source page (re-checked on each navigation into the page and after `acceptConsent`). Adding a new risk service = one registry entry + `data-risk-overlay` container on the source page + `data-risk-service` attribute on the Launcher-config checkbox |
...
| `js/damage-hook.js` | Damage multiplier page logic (status polling, multiplier/log/api fields, apply/inject/eject, force offset refresh); gated by the shared `RiskGate` risk-notice overlay |
...
| `sections/*.html` | 20 individual section HTML fragments, including `resource-updater.html` with read-only shared game path (set in 设置 page), update scope, download strategy, progress, actions, logs, and a Launcher integration intro card (switch + detailed settings on launcher-config page). Risk-service sections (`speed.html`, `input-bypass.html`, `damage-hook.html`) carry a `data-risk-overlay` container filled by `RiskGate`, plus a 查看风险须知 re-read link; `launcher-config.html` risk checkboxes carry `data-risk-service` attributes (damage_hook 的选项在未同意前由 `RiskGate.refreshLauncherVisibility()` 整组隐藏)。`launcher-config.html` 已取消独立的汉化包下载配置（零协/OurPlay/LCTA-AU 三卡），仅保留「工作模式配置」「更新集成」「游戏加速」「CDN优选」「启动增强」五张卡片 + 「汉化包下载配置」跳转卡（`goAndShow('download')`）；下载细节与「汉化包下载」页共用 `ui_default.{zero,ourplay,machine}` 一套配置 |
...
| `damage_hook.py` | `DamageHookMixin` | `damage_hook_*` 全部（get_status/apply/inject/eject/refresh_offsets，转发到 `webutils.function_damage_hook.DamageHookManager`） |
...
| `function_damage_hook.py` | 伤害倍率 (MinHook damage multiplier) | Injects `hooks/damage_hook.dll` (vendored MinHook v1.3.4) into `LimbusCompany.exe`; detours `BattleUnitModel$$GetTakeAttackDmgMultiplier` so damage taken by enemies is multiplied by `multiplier` (default 3.0, clamped [0.1, 1000]). **Offsets come from a JSON API** (default `https://web.lcta.top/damage_hook.json`, overridable via `launcher.work.damage_hook_api`) with local caching under `%LOCALAPPDATA%/LCTA/damage-hook/`: cache key = local `GameAssembly.dll` SHA-256 (140MB hash recomputed only when mtime/size change via `local-meta.json`); mismatch (game updated) auto-refetches; if the API hasn't published the new version yet it keeps the old cache and degrades with a `stale` flag. Runtime backstop: the DLL verifies the 16-byte prologue before hooking and reports `verified=0`, after which the manager force-refreshes offsets and sets `retry_requested` so the DLL hot-reinstalls without restarting the game. Config shared via `Local\LCTA_DamageHook_Config` (16584-byte `DHConfig` matching the C struct). **Damage log** (`damage_hook_log`): DLL writes each effective damage event (`target/attacker/crit/mul before -> after`) into a shared-memory ring buffer (`log_ring[128][128]`, monotonic `log_head`); a background drain thread (0.5s, started by `_open_map`, stopped+flushed by `close()`/atexit) pulls new entries via `drain_new_log_entries()` and writes them through `LogManager` into `logs/app.log` (overflow → dropped warning); page "最近日志" still shows only the latest entry. Manager API: `resolve_offsets(force_refresh)`/`apply()`/`inject(pid)`/`eject()`/`get_status()`/`refresh_offsets()`/`close()`; pure helpers `parse_multiplier`/`parse_prologue_bytes`/`validate_payload`/`build_config`/`drain_new_log_entries`. See `docs/DAMAGE_HOOK.md` |
...
| `game_launch.py` | Game launch phases: `prepare_mod()` (mod patching pre-game), `cleanup_mod_assets()` (post-game restore), `start_speed_hotkey()` / `stop_speed_hotkey()` (lifecycle wrappers), `start_input_bypass()` / `start_damage_hook()`（后台线程等待游戏进程后注入；`start_damage_hook` 注入前校验 `damage_hook.disclaimer_accepted` 同意态，未同意则跳过）。Game process launch moved to `main.py` pipeline |
