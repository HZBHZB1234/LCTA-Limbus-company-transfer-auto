...
| `js/cg.js` | 加载页 CG 替换页控制器：`CgPage`（init 直接 `_showMain()` 显示主内容并 `refreshStatus()`，导航每次进入重刷游戏运行态/存档槽/密钥可用性，无风险门）。`refreshModel()` 切换存档槽自动 `cg_read` 渲染解锁池/锁定列表 chip（锁定 chip 带 × 移除）；锁定操作即时写入：`addLock/removeLock/clearLock` 计算新列表调 `cg_apply`（重复 ID 提示、清空二次确认）。`scanCgIds()` 经 `ProgressModal` + `cg_scan_ids(modal.id)` 后台扫描，结果列表支持文本过滤（150ms 防抖）+ 缓存状态筛选（`cg-cache-filter` 全部/仅已缓存/仅未缓存，基于 `_scanUncached` 集合）并**滚动分块渲染**（窗口 200 行、距底追加、尾行「已显示 X / Y」状态、空结果占位提示），点击条目 `_selectCg` 联动输入框并 `previewCg`（`cg_preview` → data URI 渲染）。贴图替换：`replaceTexture`（确认弹窗 + `cg_replace` 模态进度，成功后记录 modded 显示「还原原图」）/ `restoreTexture`（`cg_restore`） |
