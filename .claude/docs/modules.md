...
| `js/metadata-recovery.js` | Metadata 恢复页控制器：`MetadataRecoveryPage`（init 绑定事件 + 刷新插件/输出目录状态，导航每次进入重刷；安装插件/浏览目录/开始运行）。**结构化流程**：步骤徽标**动态状态机**（`updateStepBadges`：init→步骤1、installPlugin 成功→步骤3、renderExport 成功→步骤6、run→步骤6；完成徽标绿色填充，替代原静态 current）；`loadExport()`（步骤 3：路径 + 候选 rank → `metadata_recovery_load_export` → `renderExport` 填充候选下拉（`#rank name (score)`，无文本的标记「无反编译文本」）+ 信息区（verdict 徽标/hex/文本就绪状态）+ 自动回填 textarea/hex/反编译文件输入，回填后步骤 4/5 卡片 2 秒高亮 + 「已自动填充」标签 + 解释 decompile-file 清空原因；候选下拉未载入时 disabled 占位「请先在上方载入导出」）。运行经 `ProgressModal` + `metadata_recovery_run(config, modal.id)`，结果渲染各阶段 verdict 徽标（PASS/PASS_WITH_REVIEW/FAIL/SKIP + 中文副标签 + 图例行 + 按失败阶段输出排查建议）、输出文件「用途：文件名」列表并标注 ★ 正式 profile；运行期间按钮禁用「运行中…」；「打开输出目录」按钮在目录缺失/失败时禁用并用实际 run_dir；输入校验（必填 metadata、反编译文本或既有 profile、table hex/SHA 格式）；未配置游戏路径时渲染 `goAndShow('settings')` 跳转按钮 |
