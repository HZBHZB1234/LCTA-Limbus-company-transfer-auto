...
> 状态：执行中。2026-08-06 已开始首个“共享双 WebView + 首页 + Launcher 会话 + 推荐汉化安装”纵切片。
>
> 本文件定义目标、边界、阶段、任务与验收标准；用户已确认开始执行，当前按首个纵切片推进。

> 当前实现规格与已知缺口记录在 `docs/superpowers/specs/2026-08-06-product-ux-design.md`。
...
### 17.1 2026-08-06 实施进度

- [x] 建立 `WorkspaceSnapshot`、`ActionPlan`、统一任务和 `LaunchSessionSnapshot` 第一版。
- [x] 建立 Vue 3 + TypeScript + Pinia + Vite 双入口并生成 `webui/product/`。
- [x] 主 WebUI 默认进入新首页，旧 UI 可通过 `--legacy-ui` 和界面入口回退。
- [x] Launcher WebView 成为默认模式，启动管线移入受控工作线程。
- [x] 实现 Launcher 阶段时间线、日志、游戏状态、关闭选择和主窗口深链接。
- [x] 实现主页面 Launcher 会话卡片与持久化会话读取。
- [x] 实现推荐 LLC 汉化动作计划、后台任务、取消和结果结构。
- [x] 本地与发布流程在 InitCode 前执行现代前端构建。
- [x] 添加 Python 回归测试、Vue 类型检查与生产构建验证。
- [ ] 在真实 WebView2 + 游戏环境完成安装、启动、关闭与失败恢复冒烟测试。
- [ ] 完成人工视觉取样、对比度记录和可用性点击数复测。
- [ ] 实现跨进程单实例窗口发现与聚焦后再关闭首个里程碑 Gate。
