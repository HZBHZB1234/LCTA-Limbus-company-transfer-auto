...
class ConfigManager:
    ...
    def load_config_types(self):
        with open(pathManager.code_path / "config_check.json", 'r', encoding='utf-8') as f:
            self.check = json.load(f)
        
    def load_config(self):
        with suppress(FileNotFoundError):
            with open( pathManager.work_path / "config.json",'r',encoding='utf-8') as f:
                self.config = json.load(f)

    def load_config_default(self):
        with open( pathManager.code_path / "config_default.json",'r',encoding='utf-8') as f:
            self.default = json.load(f)
    
    def save_config(self):
        with open( pathManager.work_path / "config.json",'w',encoding='utf-8') as f:
            json.dump(self.config, f)

    def validate_config_value(self, value: Any, expected_type: Union[str, List[Any]]) -> bool:
        """
        验证单个配置值的类型
    ...
    def validate_config(self, config: Dict[str, Any], config_types: Optional[Dict[str, Any]]=None) -> Tuple[bool, List[str]]:
        """
        验证配置对象是否符合类型定义
    ...
    def check_config_type(self, key_path: str, value: Any) -> Tuple[bool, Union[str, Dict[str, Any]], str]:
        """
        检查特定配置项的类型
    ...
    def fix_config(self, config: Dict[str, Any], config_default: Optional[Dict[str, Any]]=None, config_check: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """
        修复配置文件中的错误
        ...
        if config_default is None:
            config_default = self.default
        
        if config_check is None:
            config_check = self.check
        
        if config_default is None or config_check is None:
            logManager.warn("警告: 无法加载默认配置或配置类型定义，跳过配置修复")
            return config
                ...
                if isinstance(expected_type, dict) and isinstance(value, dict):
                    ...
                    if not self.validate_config_value(value, expected_type):
                        ...
                        if key in current_default and self.validate_config_value(current_default[key], expected_type):
                            current_config[key] = current_default[key]
                            logManager.info(f"修复配置: 修正键 '{current_path}' 的值为默认值 {current_default[key]}")
                        else:
