...
const TOOLTIP_DATA = {
    ...
    'dump-translation': '将完整提示词、AI 原始响应、HTTP 错误正文、解析原因与异常堆栈写入诊断文件。内容可能包含待翻译文本，但认证信息会自动脱敏。',
    'enable-concurrent': '开启后由 Rust/Tokio 同时调度多个文件、HTTP 请求和文件操作。关闭后三个并发限制都降为 1，适合排查服务端限流或文件冲突。',
    'file-concurrency': '同时处理的翻译文件数量。默认 24；文件较多时可提高吞吐，但也会增加内存占用和待发送请求数量。',
    'request-concurrency': '同时进行的 HTTP 翻译请求数量，直接影响网络吞吐和 API 限流风险。默认 16；出现 429 或服务不稳定时应降低。',
    'file-io-concurrency': '同时执行的 Rust 异步文件读取、解析和写入任务数量。默认 32；机械硬盘建议降低，SSD 可适度提高。',
    'enable-self-check': '主翻译完成后额外发送一次 LLM 自检请求，修正明显错误。可提升质量，但会增加 API 调用次数、耗时和费用。',
    'enable-thinking': '向兼容服务请求推理模式。可能提升复杂文本质量，但会增加响应延迟和 token 消耗；是否生效取决于所选模型。',
    'enable-rule-validation': '使用 Rust 确定性规则校验括号、效果引用、标签、占位符和数字等结构，并对失败单元发起补充翻译。',
