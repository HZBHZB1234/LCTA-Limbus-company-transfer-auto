        ...
        button.addEventListener('click', () => {
            ...
            if (section) {
                setTimeout(async () => {
                    ...
                    if (sectionId === 'resource-updater-section' && typeof resourceUpdaterPage !== 'undefined') {
...
const TOOLTIP_DATA = {
    ...
    'enable-rule-validation': '在 Stage 1 翻译完成后、Stage 2 LLM 校验前，使用确定性规则引擎检测并自动修复机械性错误（如 Buff ID 中误用空格、特殊效果引用缺失）。仅对技能文件生效。',
    ...
    'launcher-work-ui-mode': '推荐使用现代 WebView GUI。兼容 GUI 与控制台模式仅用于旧环境或界面启动失败时回退。',
