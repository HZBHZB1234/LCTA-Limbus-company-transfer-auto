...
def bank_password(bank_path: str, explicit: Optional[str] = None) -> Optional[str]:
    if explicit is not None:
        return explicit or None
    bank_dir = os.path.dirname(os.path.abspath(bank_path))
    base = bank_base(bank_path)
    bank_txt = os.path.join(bank_dir, base + ".txt")
...
def extract_bank(dlls: FmodDlls, bank_path: str, wav_dir: str, fsb_dir: str,
                 password: Optional[str] = None, log=None) -> dict:
    ...
    password_used = None
    if encrypted:
        password_used = bank_password(bank_path, password)
        if not password_used:
            raise BankToolError("bank 已加密但找不到密码（%s.txt / password.txt / 手动输入）" % base)
    ...
    for j in range(info["fsb_count"]):
        ...
        try:
            dlls.decode_fsb_to_wav(fsb_path, sub, "%s[%d]" % (base, j), password_used, log)
        except BankToolError as e:
            if log:
                log("[警告] 跳过打不开的 FSB %s[%d]: %s" % (base, j, e))
    return {"bank_base": base, "fsb_count": info["fsb_count"],
            "encrypted": encrypted, "password_used": password_used}
...
def rebuild_bank(dlls: FmodDlls, bank_path: str, wav_dir: str, fsb_dir: str, build_dir: str,
    ...
    for j in range(info["fsb_count"]):
        ...
        dlls.encode_wavs_to_fsb(wav_files, fsb_out, options["format"], options["quality"],
                                options["threads"], options["cache_dir"], password, log)
