...
def _LCTA_auto_github(modal_id, logger_: LogManager, use_proxy) -> str:
    ...
    if r:
        logger_.ModalLog("下载完成", modal_id)
        logger_.ModalStatus("下载完成", modal_id)
        return assets.name
    else:
        logger_.ModalLog("下载失败", modal_id)
        logger_.ModalStatus("下载失败", modal_id)
...
def _LCTA_auto_api(modal_id, logger_: LogManager) -> str:
    ...
    logger_.ModalLog(f"下载地址获取完毕 {download_url}", modal_id)
    logger_.modalProgress(30, "下载地址获取完毕", modal_id)
    
    logger_.ModalLog("开始下载", modal_id)
    logger_.ModalStatus("正在下载", modal_id)
