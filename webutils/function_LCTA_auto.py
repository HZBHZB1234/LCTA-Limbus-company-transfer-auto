...
def _LCTA_auto_github(modal_id, use_proxy) -> str:
    ...
    if r:
        LogManager().log_modal_process("下载完成", modal_id)
        LogManager().log_modal_status("下载完成", modal_id)
        return assets.name
    else:
        LogManager().log_modal_process("下载失败", modal_id)
        LogManager().log_modal_status("下载失败", modal_id)
...
def _LCTA_auto_api(modal_id) -> str:
    ...
    LogManager().log_modal_process(f"下载地址获取完毕 {download_url}", modal_id)
    LogManager().update_modal_progress(30, "下载地址获取完毕", modal_id)
    
    LogManager().log_modal_process("开始下载", modal_id)
    LogManager().log_modal_status("正在下载", modal_id)
...
def function_LCTA_auto_main(modal_id, whole_config):
