...
def _exec_re(data: dict, argu: dict) -> dict:
    aim_pattern = re.compile(argu['aim'])
    re_from = [re.compile(i) for i in argu['from']]
    re_to: list = argu['to']

    aim = deepcopy({i: data[i] for i in data if aim_pattern.search(i)})

    def effect(text):
        r = text
        for _from, _to in zip(re_from, re_to):
            r = _from.sub(_to, text)
        return r

    aim = {i: effect(aim[i]) for i in aim}
    return aim

def exec_json(data: dict, config: list) -> dict:
    flatten_data = flatten_dict_enhanced(data)

    updates = {}
    for i in config:
        try:
            if i['type']=='re':
                current = _exec_re(flatten_data, i)
...
def fancy_main(game_path: str, package_name: str, config: list):
    ...
    for file in files:
        using_config = [i for i in config if re.search(i['aim_file'], str(file))]
        if using_config:
