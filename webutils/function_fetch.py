...
def function_fetch_main(modal_id, **kwargs):
    ...
    if output_type == 'json':
        _log_manager.log_modal_process("正在保存为json文件", modal_id)
        with open(f"proper.json", "w", encoding='utf-8') as f:
            json.dump(json_data, f, ensure_ascii=False, indent=4)
    elif output_type == 'single' or output_type == 'double':
        _log_manager.log_modal_process("正在保存为单词表文件", modal_id)
    ...
    _log_manager.log_modal_process("数据保存完成", modal_id)
    _log_manager.log_modal_status("数据保存完成", modal_id)
