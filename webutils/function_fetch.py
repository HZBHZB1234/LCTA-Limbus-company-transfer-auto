...
def function_fetch_main(modal_id, **kwargs):
    LogManager().log_modal_process("成功链接后端", modal_id)
    LogManager().log_modal_status("正在抓取数据", modal_id)
    data = []
    for i in range(10):
        r=requests.get(f"https://paratranz.cn/api/projects/6860/terms?pageSize=800&page={i+1}",timeout=10)
        r.raise_for_status()
        LogManager().log_modal_process(f"正在获取第{i+1}页数据", modal_id)
        r = r.json()
        if len(r.get('results', []))==0:
            break
        data.extend(r['results'])
    LogManager().log_modal_process("数据获取完成", modal_id)
    LogManager().log_modal_process("开始处理数据", modal_id)
    LogManager().log_modal_status("正在处理数据", modal_id)
    ...
    LogManager().log_modal_process("数据处理完成", modal_id)
    LogManager().log_modal_status("正在保存数据", modal_id)
    LogManager().log_modal_process("开始保存数据", modal_id)
    output_type = kwargs.get('output_type', 'json')
    if output_type == 'json':
        LogManager().log_modal_process("正在保存为json文件", modal_id)
        with open(f"proper.json", "w", encoding='utf-8') as f:
            json.dump(json_data, f, ensure_ascii=False, indent=4)
    elif output_type == 'single' or output_type == 'double':
        LogManager().log_modal_process("正在保存为单词表文件", modal_id)
    ...
    LogManager().log_modal_process("数据保存完成", modal_id)
    LogManager().log_modal_status("数据保存完成", modal_id)
