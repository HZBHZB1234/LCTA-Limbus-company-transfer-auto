...
def function_llc_main(modal_id, logger_: LogManager, **kwargs):
    logger_.ModalLog("成功链接后端", modal_id)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        logger_.ModalLog("开始下载翻译文件", modal_id)
        logger_.ModalStatus("正在初始化链接", modal_id)
        logger_.info(f'配置信息: {str(kwargs)}')

        # 根据from_参数选择不同的下载源
        if from_ == 'api':
            logger_.ModalLog("使用API模式下载", modal_id)
            ...
            logger_.ModalLog("开始请求版本", modal_id)
            logger_.ModalStatus("正在请求版本", modal_id)
            ...
            if not last_zip:
                logger_.ModalLog("未找到合适的版本文件", modal_id)
                raise Exception("未找到合适的版本文件")

            last_zip = last_zip[0]
            logger_.check_running(modal_id)

            logger_.ModalLog("版本请求完毕", modal_id)
            logger_.modalProgress(20, "版本请求完毕", modal_id)

            save_path_text = f"{temp_dir}/{last_zip.name}"
            logger_.info(f"下载地址获取完毕 {last_zip.download_url}")

            logger_.ModalLog("开始下载文本文件", modal_id)
            logger_.ModalStatus("正在下载文本文件", modal_id)
            ...
            if not download_with_github(
                ...
                logger_.ModalLog("下载文本文件时出现错误", modal_id)
                raise

            logger_.info("文本文件下载完成")
            logger_.ModalLog("文本文件下载完成")

            save_path_font = f"{temp_dir}/LLCCN-Font.7z"
            
            if not use_cache:
                logger_.ModalLog("开始下载字体文件", modal_id)
                logger_.ModalStatus("正在下载字体文件", modal_id)
                ...
                if not download_with_github(
                    ...
                    logger_.ModalLog("下载字体文件时出现错误", modal_id)
                    raise
                
                logger_.info("字体文件下载完成")
                logger_.ModalLog("字体文件下载完成", modal_id)
            
            logger_.ModalStatus("保存文件", modal_id)
            
            if dump_default:
                logger_.info("dump_default")
                logger_.ModalLog("检测到设置：保存原文件", modal_id)
                shutil.copy2(save_path_text, last_zip.name)
                logger_.ModalLog("保存文本文件成功", modal_id)
                
                if not use_cache:
                    shutil.copy2(save_path_font, "LLCCN-Font.7z")
                    logger_.ModalLog("保存字体文件成功", modal_id)
                
                logger_.modalProgress(100, "文件保存完成", modal_id)
            else:
                logger_.info("make_zip")
                logger_.ModalLog("开始解压文件", modal_id)
                logger_.ModalStatus("正在解压文件", modal_id)

                decompress_by_extension(save_path_text, temp_dir, logger_=logger_)
                logger_.modalProgress(80, "成功解压文本文件", modal_id)
                logger_.ModalLog("成功解压文本文件", modal_id)
                logger_.check_running(modal_id)

                if not use_cache:
                    decompress_by_extension(save_path_font, temp_dir, logger_=logger_)
                    logger_.modalProgress(90, "成功解压字体文件", modal_id)
                    logger_.ModalLog("成功解压字体文件", modal_id)
                    logger_.check_running(modal_id)
                else:
                ...
                logger_.ModalStatus("正在打包文件", modal_id)
                final_zip_path = last_zip.name.replace(".7z", ".zip")
                logger_.ModalLog("开始重新打包文件", modal_id)
                
                lang_path = f'{temp_dir}\\LimbusCompany_Data\\Lang\\LLC_zh-CN'
                if not zip_folder(lang_path, final_zip_path, logger_=logger_):
                    logger_.ModalLog("打包文件时出现错误", modal_id)
                    raise
                
                logger_.modalProgress(100, "成功打包文件", modal_id)
                logger_.ModalLog("成功打包文件", modal_id)
                logger_.ModalStatus("全部操作完成", modal_id)
...
def _download_from_api(temp_dir, logger_: LogManager, modal_id, zip_type,
    ...
    try:
        ...
        logger_.ModalLog("API数据解析失败", modal_id)
    ...
    if not download_url:
        logger_.ModalLog("未能从API获取有效的下载链接", modal_id)
    ...
    logger_.check_running(modal_id)
    logger_.ModalLog(f"版本请求完毕: {version}", modal_id)
    logger_.modalProgress(20, "版本请求完毕", modal_id)
    ...
    logger_.info(f"下载地址获取完毕 {download_url}")
