...
def check_ver_ourplay(logger_: LogManager = None):
    ...
    logger_.info("正在请求 OurPlay 汉化包信息")
...
def download_ourplay(logger_: LogManager = None):
    ...
    logger_.info(f"OurPlay 响应: {str(response_data)}")
    
    if response_data['code'] != 1:
        logger_.info("OurPlay 请求失败")
        return None
    ...
    logger_.info(f"OurPlay 下载信息: URL={download_url}, MD5={md5}, Size={size}")
...
def function_ourplay_main(modal_id, logger_: LogManager, **kwargs):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        logger_.ModalLog("开始下载 OurPlay 汉化包", modal_id)
        logger_.ModalStatus("正在初始化链接", modal_id)
        
        # 获取下载信息
        download_info = download_ourplay(logger_)
        if not download_info:
            logger_.ModalLog("获取 OurPlay 下载信息失败", modal_id)
            raise
            
        url, expected_md5, size = download_info
        save_path = f"{temp_dir}/transfile.zip"
        
        logger_.info(f"OurPlay 下载地址: {url}")
        logger_.check_running(modal_id)
        logger_.ModalLog("开始下载汉化包", modal_id)
        logger_.ModalStatus("正在下载汉化包", modal_id)
        
        # 下载文件
        if not download_with(url, save_path, size=size, chunk_size=1024*100, 
                            logger_=logger_, modal_id=modal_id, progress_=[0, 50]):
            logger_.ModalLog("下载 OurPlay 汉化包时出现错误", modal_id)
            raise
            
        logger_.info("OurPlay 汉化包下载完成")
        logger_.ModalLog("OurPlay 汉化包下载完成", modal_id)
        logger_.modalProgress(50, "下载完成", modal_id)
        
        # 验证 MD5
        hash_ok = True
        if kwargs.get("check_hash"):
            logger_.ModalLog("正在验证文件完整性", modal_id)
            logger_.ModalStatus("正在验证文件", modal_id)
            
            actual_md5 = calculate_md5(save_path, logger_)
            if not actual_md5:
                logger_.ModalLog("计算文件 MD5 失败", modal_id)
                hash_ok = False
                
            if actual_md5 != expected_md5:
                hash_ok = False
                logger_.ModalLog(f"文件校验失败，期望值: {expected_md5}, 实际值: {actual_md5}", modal_id)
                
            logger_.ModalLog("文件校验通过", modal_id)
            logger_.modalProgress(60, "文件校验完成", modal_id)
        else:
            logger_.ModalLog("跳过文件校验", modal_id)
            logger_.modalProgress(60, "跳过文件校验", modal_id)
        
        logger_.check_running(modal_id)
        logger_.ModalLog("开始处理文件", modal_id)
        logger_.ModalStatus("正在处理文件", modal_id)
        ...
        if font_option == "llc":
            ...
            logger_.info("已替换为缓存字体")
                
        logger_.info("正在压缩文件...")
        if not zip_folder(f'{temp_dir}\\com.ProjectMoon.LimbusCompany\\Lang\\OurPlayHanHua', 'ourplay.zip', logger_):
            logger_.ModalLog("处理文件时出现错误", modal_id)
            raise
            
        logger_.info('格式化完成')
        logger_.ModalLog("文件处理完成", modal_id)
        logger_.modalProgress(100, "操作完成", modal_id)
        if not hash_ok:
            logger_.ModalLog("操作完成，但存在哈希校验失败的文件，请注意使用风险", modal_id)
            logger_.ModalStatus("操作完成  警告：存在哈希校验失败的文件")
        else:
            logger_.ModalStatus("全部操作完成")
